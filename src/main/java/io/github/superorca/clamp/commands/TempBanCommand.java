...
@Command("tempban")
@CommandPermission("clamp.tempban")
public class TempBanCommand {
    private final Clamp plugin;

    public TempBanCommand(Clamp plugin) {
        this.plugin = plugin;
    }
    ...
    public void execute(CommandSender sender, OfflinePlayer target, Duration timespan, @Optional String reason, @Switch("s") boolean silent) {
        ...
        Punishment punishment = new Punishment(plugin, plugin.generateId(), Date.from(Instant.now()), PunishmentType.BAN, target, mod, reason, timespan, true);
        punishment.write();
