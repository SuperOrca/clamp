...
@Command("mute")
@CommandPermission("clamp.mute")
public class MuteCommand {
    private final Clamp plugin;

    public MuteCommand(Clamp plugin) {
        this.plugin = plugin;
    }
    ...
    public void execute(CommandSender sender, OfflinePlayer target, @Optional String reason, @Switch("s") boolean silent) {
        ...
        Punishment punishment = new Punishment(plugin, plugin.generateId(), Date.from(Instant.now()), PunishmentType.MUTE, target, mod, reason, null, true);
        punishment.write();
